...
<head>
    ...
    <title>@yield('title', 'Dashboard Dosen Wali')</title>
        ...
        <aside class="w-64 bg-gray-800 text-white p-4 hidden md:block">
            <div class="text-2xl font-bold mb-6">
                ...
                @if (Auth::user()->role === 'dosen_wali')
                    <h2>
                        @if (Auth::user()->name === 'Dosen Wali 1')
                            Dosen Wali 1
                        @elseif(Auth::user()->name === 'Dosen Wali 2')
                            Dosen Wali 2
                        @endif
                    </h2>
                @endif
            ...
            <nav>
                <nav>
                    <ul>
                        <li class="mb-4">
                            <a href="{{ route('layout.dosendasbor') }}"
                                class="flex items-center hover:bg-gray-700 p-2 rounded {{ request()->routeIs('layout.dosendasbor') ? 'bg-gray-700' : '' }}">
                        ...
                        <li class="mb-4">
                            <a href="{{ route('mahasiswa.index') }}"
                                class="flex items-center hover:bg-gray-700 p-2 rounded {{ request()->routeIs('mahasiswa.index') ? 'bg-gray-700' : '' }}">
                                <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <li class="mb-4">
                            <a href="{{ route('dosen.index') }}"
                                class="flex items-center hover:bg-gray-700 p-2 rounded {{ request()->routeIs('dosen.index') ? 'bg-gray-700' : '' }}">
                                <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6">
